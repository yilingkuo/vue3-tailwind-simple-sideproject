...
<template>
  <div class="">
    ...
    <div
      class="mx-auto max-w-screen-xl p-4 sm:px-6 flex items-center justify-center  lg:p-6"
    >
      ...
      <div v-else >
        <div class="flex justify-end gap-1">
          ...
          <ViewGridIcon @click="cardView = true" :class="cardView? 'bg-sky-500': ''" class="col-1 w-10 h-10 text-gray-100 rounded pointer-cursor hover:bg-sky-700/50"/>
          <ViewListIcon @click="cardView = false" :class="cardView? '': 'bg-sky-500'" class="col-1 w-10 h-10 text-gray-100 rounded hover:bg-sky-700/50"/>
        </div>
        <div class="">
          <div v-if="cardView" class="w-full gap-6 grid grid-cols-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          ...
          <div v-else class="gap-6 ">
            <List v-show="!cardView" v-for="(user, index) in currentPageItems" :key="index" :profile-photo="user.picture.large" :name="user.name" :detail="user" />
          </div>
        </div>
