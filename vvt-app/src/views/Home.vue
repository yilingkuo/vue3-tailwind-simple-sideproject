...
const randomusers = methods.getApi('https://randomuser.me/api/?results=10').then(r => {
...
<template>
  <div class="bg-gray-200">
    ...
    >
      ...
      <div v-else class=" grid grid-cols-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          <!-- <p>{{user.name}}</p>
          {{user.picture.medium}} -->
        <Card  v-for="(user, index) in userdata" :key="index" :profile-photo="user.picture.large" :name="user.name" />
      </div>
    </div>
  </div>
</template>
